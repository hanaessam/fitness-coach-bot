...
class GymAnalytics:
    ...
    def get_benchmark(
        ...
        # Age range (±10 years)
        filtered = filtered[
            (filtered["Age"] >= age - 10) & (filtered["Age"] <= age + 10)
        ]
        ...
        if len(filtered) == 0:
            return {"found": False, "message": "No similar users found"}
        ...
        return {
            ...
            "avg_session_duration": round(
                filtered["Session_Duration (hours)"].mean(), 2
            ),
            "avg_calories_burned": round(filtered["Calories_Burned"].mean(), 0),
            "avg_workout_frequency": round(
                filtered["Workout_Frequency (days/week)"].mean(), 1
            ),
            "avg_bmi": round(filtered["BMI"].mean(), 1),
            "percentile_25_calories": round(
                filtered["Calories_Burned"].quantile(0.25), 0
            ),
            "percentile_50_calories": round(
                filtered["Calories_Burned"].quantile(0.50), 0
            ),
            "percentile_75_calories": round(
                filtered["Calories_Burned"].quantile(0.75), 0
            ),
        }

    def recommend_workout_type(
        self, age: int, gender: str, goal: str, experience_level: int
    ) -> Dict:
                ...
                recommendations.append(
                    {
                        ...
                        "confidence": (
                            "high" if benchmark["sample_size"] > 5 else "medium"
                        ),
                    }
                )
    ...
    def predict_calories(
        ...
        avg_calories_per_hour = (
            filtered["Calories_Burned"].mean()
            / filtered["Session_Duration (hours)"].mean()
        )
